...
Validates date continuity, spend columns, negative values, and pairwise correlation.
"""
...
def precheck_dataset(
    ...
    if unique_dates < 52:
        findings.append({
            ...
            "severity": "warning",
            "message": f"Only {unique_dates} unique dates found. 52+ weeks strongly recommended."
        })
        ...
        for r, c in zip(*high_corr):
            ...
            if pair not in seen_pairs:
                ...
                findings.append({
                    ...
                    "severity": "warning",
                    "message": f"Channels '{pair[0]}' and '{pair[1]}' have correlation r={corr_matrix.iloc[r, c]:.2f} >= 0.90."
                })
...
def main():
    parser = argparse.ArgumentParser(description="Pre-check marketing dataset for MMM.")
