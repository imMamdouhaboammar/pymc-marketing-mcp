...
| `pymc-marketing` | `>=1.1.0,<2` | MMM, incrementality, optimizer and CLV computation boundary |
...
| `pyjwt` | `>=2.8.0,<3` | JWT validation primitives |
| `httpx` | `>=0.27,<1` | HTTP client for remote communication and testing |
...
The dependency range for `pymc-marketing` is declared as `>=1.1.0,<2` in `pyproject.toml`, bounding the supported range to PyMC-Marketing 1.x minor releases while guarding against breaking 2.x changes.
