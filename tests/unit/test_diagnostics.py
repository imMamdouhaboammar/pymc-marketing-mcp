...
def test_diagnostics_rejects_divergent_sampler():
    idata=FakeIdata()
    idata["sample_stats"]=xr.Dataset({"diverging":(("chain","draw"),np.ones((2,20),dtype=int))})
    idata["posterior"]=xr.Dataset({"beta":(("chain","draw"),np.random.default_rng(1).normal(size=(2,20)))})
